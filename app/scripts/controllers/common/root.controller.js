var debugMode = true;
app.controller("RootCtrl", ['$rootScope', '$scope', '$location', '$log','webServices', 'localStorageService', 'consts', 'env', function($rootScope, $scope, $location, $log, webServices, localStorageService, consts, env){
	...
	/****************************************** SAVE *********************************************************/
	$scope.save = function(){
		localStorageService.set(env.ident + "_session", $rootScope.session);
	};
	$scope.load = function(){
		return localStorageService.get(env.ident + "_session");
	};
	$scope.hasSave = function(){
		return localStorageService.get(env.ident + "_session") != null;
	};
	/****************************************** USER *********************************************************/
	...
	$scope.logOut = function(){
		webServices.logout($scope.getUserId(), function(){
			$location.path( "/login" );
			localStorageService.set(env.ident + "_session", null);
			$scope.$apply();
		});
	...
	$scope.setCharacter = function(character){
		var avatars = character.avatarId % 2 != 0? consts.avatarMan : consts.avatarWoman;
